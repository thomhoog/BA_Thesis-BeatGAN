...
def make_generator_model():
    ...
    model.add(layers.Conv2DTranspose(1, (5,5), strides=(2,2), use_bias=True, padding='same', activation='tanh'))
...
main_dir = os.path.join(parentDir, 'dataset', 'groove_img (16x16)')
batch = 32
epochs = 100
...
opt_dis = tf.keras.optimizers.SGD(0.01, momentum=0.9)
opt_gen = tf.keras.optimizers.SGD(0.01, momentum=0.9)
...
def train_loop():
    for epoch in range(epochs):
        ...
        tf.print(total_d_loss_real/280)
        print(f"Discriminator fake loss, epoch {epoch + 1}")
        tf.print(total_d_loss_fake/280)
        print(f"Generator loss, epoch {epoch + 1}")
        tf.print(total_g_loss/280)
